...
public class NinepClient {
	...
	private static Styx ninep = new Styx();
	...
	private class Request {
		...
		Request(Ninep.Tmsg t){ this.t = t; }
		Request(Ninep.Tmsg t, Request old){ this.t = t; flushed = old; }
	...
	private class Requests {
		...
		synchronized final void add(Request req){
			if(err == null){
				req.next = set;
				set = req;
			}else if(req.t instanceof Ninep.Tflush)
				req.r = ninep.new Rflush(req.t.tag);
			else
				req.r = ninep.new Rerror(req.t.tag, err);
		}
...
System.out.println("shut down: "+why);
			synchronized(this){
				...
				while((r = set) != null){
					...
					r.next = null;
					synchronized(r){
						if(r.t instanceof Ninep.Tflush)
							r.r = ninep.new Rflush(r.t.tag);
						else
							r.r = ninep.new Rerror(r.t.tag, why);
						r.notify();
System.out.println("notify: "+r.t.tag+" "+r.t.mname());
	...
	public class Conn {
		...
		protected Ninep.Rmsg ninepreq(Ninep.Tmsg f){
			int otype = f.mtype();
			int tag;
			if(otype == Ninep.MTversion)
				f.tag = tag = Ninep.NOTAG;
			...
			for(;;){
				...
				try{
					...
					req = new Request(ninep.new Tflush(oldtag), req);	// ie, flush original request, not last Tflush
		...
		private Request flushout(Request req){
			for(;;){
				if(req.r == null)
					active.completed(ninep.new Rflush(req.t.tag));
				if(req.flushed == null)
					break;
				req = req.flushed;
			}
	...
	public class FD {
		...
		public ByteBuffer read(int n, long off){
			...
			Ninep.Rread r = (Ninep.Rread)conn.ninepreq(ninep.new Tread(fid, off, n));
			if(r == null)
				return null;
		...
		public int write(ByteBuffer buf, int n, long off){
			...
			Ninep.Rwrite r = (Ninep.Rwrite)conn.ninepreq(ninep.new Twrite(fid, off, buf, n));
			if(r == null)
				return -1;
			return r.count;
		}
		...
		public Dir[] dirread(){
			...
			if(b == null)
				return null;
			ArrayList v = new ArrayList(b.remaining()/Ninep.STATFIXLEN);
			dirunpack(b, v);
			return dirents(v);
		}
